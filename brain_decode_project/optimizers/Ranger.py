...
class Ranger(Optimizer):
    ...
    def __init__(self, params, lr=1e-3,                          # lr
        ...
        self.logger = logging.getLogger('RangerOptimizer')
        # parameter checks
        ...
        self.logger.debug(f"Ranger optimizer loaded. \nGradient Centralization usage = {self.use_gc}")
        if self.use_gc and not self.gc_conv_only:
            self.logger.debug(f"GC applied to both conv and fc layers")
        elif self.use_gc and self.gc_conv_only:
            self.logger.debug(f"GC applied to conv layers only")
